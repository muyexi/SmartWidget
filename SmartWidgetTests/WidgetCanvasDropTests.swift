...
@Suite("WidgetCanvas drop resolution")
struct WidgetCanvasDropTests {
    ...
    func dropOnEmptyCanvas() {
        let result = WidgetCanvas().inserting(dropped, at: CGPoint(x: 130, y: 40), in: canvas)

        #expect(result == WidgetCanvas(.widget(dropped)))
    }

    @Test("A canvas with no area still accepts the first widget")
    func dropOnZeroSizedCanvas() {
        let result = WidgetCanvas().inserting(dropped, at: .zero, in: .zero)
    ...
    func splitOnTheLeading() {
        let layout = WidgetCanvas(.widget(a))
        let result = layout.inserting(dropped, at: CGPoint(x: 40, y: 100), in: canvas)

        #expect(result == WidgetCanvas(.row(.widget(dropped), .widget(a))))
    }
    ...
    func splitOnTheTrailing() {
        ...
        #expect(result == WidgetCanvas(.row(.widget(a), .widget(dropped))))
    }
    ...
    func splitOnTheTop() {
        ...
        #expect(result == WidgetCanvas(.column(.widget(dropped), .widget(a))))
    }
    ...
    func splitOnTheBottom() {
        ...
        #expect(result == WidgetCanvas(.column(.widget(a), .widget(dropped))))
    }
    ...
    func joinColumnInTheMiddle() {
        ...
        #expect(result == WidgetCanvas(.column(.widget(a), .widget(dropped), .widget(b))))
    }
    ...
    func nestsInsideAColumn() {
        let layout = WidgetCanvas(.row(.widget(a), .column(.widget(b), .widget(dropped))))
    ...
    func dropAboveCanvas() {
        ...
        #expect(result == WidgetCanvas(.column(.widget(dropped), .widget(a), .widget(b))))
    }
    ...
    func dropBelowCanvas() {
        ...
        #expect(result == WidgetCanvas(.column(.widget(a), .widget(b), .widget(dropped))))
    }
    ...
    func dropPastRightEdge() {
        ...
        #expect(result == WidgetCanvas(.row(.widget(a), .widget(b), .widget(dropped))))
    }
